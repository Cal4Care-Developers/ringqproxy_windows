...
=================================================================================
 ...
 Install: C:\Program Files\RingQProxy
...
$InstallDir     = "C:\Program Files\RingQProxy"
...
$NeedFetch = $true
$VersionUrl = "https://api.github.com/repos/$RepoOwner/$RepoName/commits/main"
$RemoteHash = ""
try {
    $headers = @{ "User-Agent" = "RingQProxy-Installer" }
    $commitInfo = Invoke-RestMethod -Uri $VersionUrl -Headers $headers -TimeoutSec 15
    $RemoteHash = $commitInfo.sha.Substring(0,7)
} catch {
    Warn "Could not query GitHub API for latest commit -- will rebuild unconditionally"
}
...
if (-not (Test-Path $NssmPath)) {
    # Prefer a copy bundled in the repo (tools\nssm.exe) if present -- more
    # reliable than depending on an external download at install time.
    $bundled = Get-ChildItem -Path $env:TEMP -Filter "nssm.exe" -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1
    if ($bundled) {
        Copy-Item $bundled.FullName $NssmPath -Force
        Ok "NSSM copied from repository bundle"
    } else {
        Info "Downloading NSSM..."
        $nssmZip = "$env:TEMP\nssm.zip"
        Invoke-WebRequest -Uri "https://nssm.cc/release/nssm-2.24.zip" -OutFile $nssmZip -TimeoutSec 60
...
& $NssmPath set $ServiceName DisplayName "RingQ NX Device SIP Proxy"
