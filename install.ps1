...
$AutoApiUrl     = if ($E.ApiUrl)     { $E.ApiUrl }     else { "https://${FinalPbxDomain}:443" }
...
if ($LASTEXITCODE -ne 0) { ErrMsg "nssm install failed (exit $LASTEXITCODE)"; exit 1 }
...
# RTP relay -- required since the proxy now relays media itself (lanHalf/
# wanHalf sockets) rather than letting RTP pass phone-to-PBX directly.
# Without this rule, calls register and ring normally but have no audio,
# since inbound RTP from LAN phones to the relay would be silently
# blocked -- a confusing failure mode if you don't know this rule exists.
Add-FwRule "RingQ-rtp-relay" "Inbound" "UDP" "40000-41999"
